...
| `conflict` (поле) | 200 | окно пересекается — **предупреждение, не блок**: `claim-check`/`overlay-claim` возвращают `conflict`, но приём всё равно проходит (§3.3–3.4) |
| `ALREADY_CLAIMED` | 400 | `overlay-claim` чужого активного заказа — **единственный жёсткий запрет** |
