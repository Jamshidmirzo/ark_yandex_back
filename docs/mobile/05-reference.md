...
| `INVALID_TRANSITION` | 400 | `trip-state`: прыжок мимо этапа / `completed` не из `at_destination` (§3.6) |
| `TOO_FAR` | 400 | `trip-state` `at_client`/`at_destination`: водитель дальше 100 м от точки (геозона) |
| `NO_FRESH_GPS` | 400 | `trip-state` прибытие без свежего GPS — стримь позицию (§04) |
| `INVALID_STATUS` | 400 | смена `trip_state` у завершённого заказа |
| `403` (PERMISSION_DENIED) | 403 | `trip-state` двигает не назначенный водитель |
