...
> 🌐 English version: [docs/mobile/en/](en/README.md)
...
3. [Расписание и оверлей](03-scheduling-overlay.md) — **этапы поездки** (как стартовать и вести), приём заказа, авто-расчёт маршрута/длительности, «1 водитель = 1 активный заказ», авто-распределение.
...
6. [WebSockets](06-websockets.md) — **все сокеты**: драйверский GPS-сокет, сокет заказа, диспетчерская, уведомления.
7. [Вебсокет водителя](07-driver-websocket.md) — **только для приложения водителя**: подключение, отправка GPS, приём маркера + полилайна, Flutter-пример.
8. [Диспетчерская](en/08-dispatcher.md) — **для веб-диспетчера** (пока только EN): топология запуска, live-лента автопарка (WS + REST), назначение/переназначение, тумблер авто-распределения, контракт live-карты.
9. [Карта заказа у заказчика](09-customer-map.md) — **для заказчика/автора**: деталь → карта, статус заказа, своё местоположение, маршрут до точки встречи, WebSocket connect/disconnect.
...
### Ввод хоста в приложении (экран Base URL)
- В приложении вводится **хост без `/api/v1`** (например `http://192.168.68.59:8000`). Дальше
  приложение само собирает пути как `host/<язык>/api/v1/...` (язык в пути — родная схема demo).
- При сохранении приложение валидирует адрес запросом **`GET host/healthcheck/`** (без авторизации) —
  ждёт **`200 {"status":"ok"}`**. Шлюз отвечает на это так же, как demo.
- Шлюз принимает **обе схемы**: фронтовую `/api/v1/...` (без языка) и мобильную
  `/<язык>/api/v1/...` — языковой префикс срезается на входе, наверх к demo всё равно уходит `/ru/`.
  То же и для healthcheck: `host/healthcheck/` и `host/ru/healthcheck/` оба дают `200`.
