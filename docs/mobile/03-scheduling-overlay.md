...
> **Долгая съёмка → не давать водителю простаивать.** Заводите как **два заказа**: «отвезти на
> съёмку» (завершается) и «забрать» позже. Середина свободна сама собой → второй заказ в «дыру»
> проходит проверку окон. Если же это один длинный заказ с ожиданием — пока водитель `waiting`,
> второй заказ можно начать (гард не мешает), но окно длинного заказа всё равно занимает время,
> поэтому модель «два заказа» предпочтительна.
